...
namespace Automatick
{
    ...
    {
        SortableBindingList<TicketsLog> all_Logs = new SortableBindingList<TicketsLog>();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (fd.ShowDialog() == DialogResult.OK)
            {
                ...
                String contents = "TicketName,Section,Row,Seat,Price,Account,FoundDateTime,MoreInfo,BuyStatus" + Environment.NewLine;
                try
                {
                    ...
                    if (this.all_Logs != null)
                    {
                        ...
                        {
                            foreach (TicketsLog item in this.all_Logs)                                                                                                                                                  //(this._selectedAccountForAutoBuy != null) ? this._selectedAccountForAutoBuy.EmailAddress : ""
                            {
                                contents += item.TicketName.Replace(",", "") + "," + item.Section.Replace(",", "") + "," + item.Row.Replace(",", "") + "," + item.Seat.Replace(",", "") + "," + item.Price.Replace(",", "") + "," + item.Account.Replace(",", "") + "," + item.FoundDateTime.ToString().Replace(",", "") + "," + (!String.IsNullOrEmpty(item.MoreInfo) ? item.MoreInfo.ToString().Replace(",", String.Empty) : "") + "," + item.BuyStatus.Replace(",", "");
