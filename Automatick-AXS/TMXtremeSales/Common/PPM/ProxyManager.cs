...
namespace ProxyManager
{
    ...
    {
        String urlSettings = "http://95.211.166.125/PPM/LicenseSettings.asmx";
        String strURL = @"http://www.ticketmaster.com";
        internal SortableBindingList<Proxy> Proxies;
        ...
        public Proxy getNextProxy()
        {
            ...
            if (this.allGroupedProxies.Count <= 0)
            {
                try
                {
                    allGroupedProxies = new List<IGrouping<string, Proxy>>(Proxies.GroupBy(x => (!String.IsNullOrEmpty(x.Address) ? ((x.Address.LastIndexOf('.') > -1) ? x.Address.Substring(0, x.Address.LastIndexOf('.')) : x.Address) : "")));
                }
                catch (Exception)
                {
                    allGroupedProxies = new List<IGrouping<string, Proxy>>();
                ...
                do
                {
                    ...
                    IEnumerable<Proxy> proxiesFiltered = from pr in this.allGroupedProxies[currentProxyIndex]
                                                         where  (pr.ProxyStatus == Proxy.proxyVerified) && 
                                                                ...
                                                                (listUsedProxies.Count(q => q == pr) < ((pr.TheProxyType == Proxy.ProxyType.MyIP) ? SettingsFromServer.MaxAllowedConnectionMyIp : (pr.TheProxyType == Proxy.ProxyType.Custom) ? SettingsFromServer.MaxAllowedConnectionCustomIP : SettingsFromServer.MaxAllowedConnectionISPIP))
                                                         select pr;
            ...
            if (selectedProxy!=null)
            {
                ...
                {
                    listUsedProxies.Add(selectedProxy);
                }
            }
        ...
        public void ReleaseProxy(Proxy proxy)
        {
            try
            {
                if (proxy!=null)
                {
                    if (listUsedProxies.Contains(proxy))
                    {
                        lock (listUsedProxies)
                        {
