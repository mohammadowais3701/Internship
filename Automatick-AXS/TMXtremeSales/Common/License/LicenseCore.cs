...
public class LicenseCore
{
    ...
    bool ifShowMessages;
    public LicenseCore(string licFilePath, bool shownMessages)
    {
    ...
    public Boolean CheckVersionUpdate()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                wc.DownloadData("https://95.211.166.125/LicensingSystem/ValidateLicense.asmx/sv?version_id=" + version_id + "&license_id=" + LicenseID);
            }


            wc = new WebClient();
            byte[] _data = wc.DownloadData(@"https://95.211.166.125/LicensingSystem/ValidateLicense.asmx/checkUpdates?license_id=" + LicenseID + "&version_id=" + version_id);
            ...
            if (result)
            {
                ...
                if (resultDialog == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        {
                            wcUpdater.DownloadFile(@"http://dl.dropbox.com/u/15557412/NSUpdater/Updater.exe", Application.StartupPath + @"\Updater.exe");
                        }
                    }
                    catch 
                    {
                    ...
                    if (!File.Exists(Application.StartupPath + @"\Ionic.Zip.dll"))
                    {
                        try
                        {
                            using (WebClient wcUpdater = new WebClient())
                            {
                                wcUpdater.DownloadFile(@"http://dl.dropbox.com/u/15557412/NSUpdater/Ionic.Zip.dll", Application.StartupPath + @"\Ionic.Zip.dll");
                            }
                        }
                        catch { }
                    }
    ...
    public Boolean ValidateLicense()
    {
        ...
        if (string.IsNullOrEmpty(LicenseCode) && ifShowMessages)
        {
            ...
            LicenseAPI.frmRequest frmReq = new LicenseAPI.frmRequest(processorID, harddiskSerial, AppPreFix, xmlServiceURL, licenseFilePath);
            frmReq.ShowDialog();
            return false;
        }
    ...
    public static XmlDocument GetServiceURL()
    {
        ...
        VALIDATESERIALNode.InnerText = "https://95.211.166.125/LicensingSystem/ValidateLicense.asmx";
        serviceURLNode.AppendChild(VALIDATESERIALNode);

        XmlElement REQUESTSERIAL = xmlServiceURL.CreateElement("REQUESTSERIAL");
        REQUESTSERIAL.InnerText = "https://95.211.166.125/LicensingSystem/LicenseRequest.asmx";
