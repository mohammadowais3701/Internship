...
namespace HtmlAgilityPack
{
    ...
    public class HtmlWeb
    {
        ...
        //private string _userAgent = "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB6.6; SLCC2; .NET CLR 2.0.50727; .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; .NET4.0C; Tablet PC 2.0; AskTB5.6)";
        //private string _userAgent = "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.4 (KHTML, like Gecko) Chrome/22.0.1229.94 Safari/537.4";
        //private string _userAgent = "Mozilla/5.0 (iPhone; U; CPU iPhone OS 3_0 like Mac OS X; en-us) AppleWebKit/528.18 (KHTML, like Gecko) Version/4.0 Mobile/7A341 Safari/528.16"; 
        private string _userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:72.0) Gecko/20100101 Firefox/72.0";//"Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:65.0) Gecko/20100101 Firefox/65.0";
        private string _referrer;
        private CookieContainer _ck = new System.Net.CookieContainer
        {
            MaxCookieSize = 8192
        };
        private Boolean _IfAllowAutoRedirect = true;
        HttpWebRequest req;
        Boolean _IsHtmlContent = false;
        ...
        /// <summary>
        /// If Auto Redirect Allowed or not.
        /// </summary>
        public Boolean IfAllowAutoRedirect
        {
            get { return _IfAllowAutoRedirect; }
            set { _IfAllowAutoRedirect = value; }
        }
        /// <summary>
        /// Cookies container
        /// </summary>
        ...
        /// <returns>A new HTML document.</returns>
        public byte[] LoadBinaryData(string url, string method, WebProxy proxy, NetworkCredential credentials)
        {
            byte[] data = null;
            Uri uri = new Uri(url);
            ...
            if ((uri.Scheme == Uri.UriSchemeHttps) ||
                (uri.Scheme == Uri.UriSchemeHttp))
            {
                data = LoadBinaryUrl(uri, method, proxy, credentials);
            }
            else
            {
                if (uri.Scheme == Uri.UriSchemeFile)
        ...
        Boolean CertificateValidationCallBack(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
        {
            return true;
        }
        ...
        private void RespCallback(IAsyncResult asynchronousResult)
        {
            ...
            try
            {
                ...
                {
                    PostResponse(req, requestState.response);
                }

                _requestDuration = Environment.TickCount - requestState.TC;
                _responseUri = requestState.response.ResponseUri;

                _IsHtmlContent = IfHtmlContent(requestState.response.ContentType);

                Encoding respenc = null;
                var isGZipEncoding = false;
                if (!string.IsNullOrEmpty(requestState.response.ContentEncoding))
                {
                    isGZipEncoding = IsGZipEncoding(requestState.response.ContentEncoding);
                    if (!isGZipEncoding)
                    {
                        respenc = Encoding.GetEncoding(requestState.response.ContentEncoding);
                    }
                }

                if (requestState.response.StatusCode == HttpStatusCode.NotModified)
                {
                    if (UsingCache)
                    {
                        _fromCache = true;
                ...
                if (isGZipEncoding)
                {
                    s = new GZipStream(requestState.response.GetResponseStream(), CompressionMode.Decompress);
                }
                else
                {
                    s = requestState.response.GetResponseStream();
                }
                ...
                if (s != null)
                {
                    ...
                    if ((requestState.hDoc != null) && (_IsHtmlContent))
                    {
                        if (respenc != null)
                        {
                            requestState.hDoc.Load(s, respenc);
                        }
                        else
                        {
                            requestState.hDoc.Load(s, true);
                        }
                    }
                    else if ((requestState.hDoc != null) && (!_IsHtmlContent))
                    {
                        requestState.hDoc.Load(s, true);
                    }
            ...
            //allDone.Set();
            requestState.allDone.Set();

        }

        private void RespCallbackForBinaryOperation(IAsyncResult asynchronousResult)
        {
            // State of request is asynchronous.
            RequestState requestState = (RequestState)asynchronousResult.AsyncState;
            ...
            try
            {
                ...
                bool html = IfHtmlContent(requestState.response.ContentType);
                ...
                if (s != null)
                {
                    requestState.binaryData = this.ReadToEnd(s);
                    requestState.response.Close();
                }
                /////////////////
            }
                                   ...
                                   ICredentials creds)
        {
            ...
            if (req.RequestUri.AbsoluteUri.Contains("GravityACS/payauth") || req.RequestUri.AbsoluteUri.Contains("GravityACS/continueWithoutPoints") && !req.RequestUri.AbsoluteUri.Contains("secure7.arcot.com"))
            {
                req.Accept = "*/*";
            }
            else if (req.RequestUri.AbsoluteUri.Contains("json"))
                req.Accept = "application/json, text/javascript, */*; q=0.01";
            else if (req.RequestUri.AbsoluteUri.Contains("unified"))
                req.Accept = "application/json";
            else
                req.Accept = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8";

            if (req.RequestUri.AbsoluteUri.Contains("axs.com"))
            {
                req.ContentType = "application/json";
                req.Accept = "application/json";
            }

            if (!req.RequestUri.AbsoluteUri.Contains("GravityACS/payauth") && !req.RequestUri.AbsoluteUri.Contains("www.securesuite.co.uk") && !req.RequestUri.AbsoluteUri.Contains("secure7.arcot.com"))
            {
                if (string.IsNullOrEmpty(this.origin))
                {
                    req.Headers.Add("Origin", "https://tix.axs.com");
                }
                else
                {
            ...
            else if (!req.RequestUri.AbsoluteUri.Contains("www.securesuite.co.uk") && !req.RequestUri.AbsoluteUri.Contains("secure7.arcot.com"))
            {
                req.Headers.Add("X-Requested-With", "XMLHttpRequest");
                req.Headers.Add("Browser", "Firefox 66");
                req.Headers.Add("Cookies-Enabled", "true");
                req.Headers.Add("Height", "1080");
                req.Headers.Add("Language", "en-US");
                req.Headers.Add("OSName", "Windows");
                req.Headers.Add("Timezone", "-300");
                req.Headers.Add("Width", "1920");
            }

            if (req.RequestUri.AbsoluteUri.Contains("veritix/pre-flow/v2"))
            {
                req.Headers.Add("FanSight-Tab", "needed");
            }

            if (req.RequestUri.AbsoluteUri.Contains("/socket.io/?EIO=3&transport=websocket"))
            {
                req.Headers.Add("Sec-WebSocket-Extensions", "permessage-deflate");
                req.Headers.Add("Sec-WebSocket-Key", "tbuwpJ/kK8NwOc2vxKM7ww==");
            ...
            req.KeepAlive = true;
            req.AllowAutoRedirect = _IfAllowAutoRedirect;
            req.ServicePoint.Expect100Continue = false;
            ServicePointManager.Expect100Continue = false;

            if (!String.IsNullOrEmpty(this.Referrer))
            {
                req.Referer = this.Referrer;
            }

            if (!String.IsNullOrEmpty(this.Authorization))
            {
                req.Headers.Set(HttpRequestHeader.Authorization, this.Authorization);
            }
            ...
            RequestState requestState = new RequestState(req, doc, tc);
            ...
            bool html = IfHtmlContent(resp.ContentType);
            ...
            if (s != null)
            {
                ...
                else
                {
                    ...
                    {
                        if (resp.ContentType.Equals("application/javascript"))
                        {
                            using (Stream stream = new GZipStream(resp.GetResponseStream(), CompressionMode.Decompress))
                            {
                        ...
                        else if (resp.ContentType.Contains("application/json") && req.RequestUri.AbsoluteUri.Contains("https://eu-west-1-machinecaptcha.queue-it.net/api/challenge/"))
                        {
        ...
        public string Get(string uri, IWebProxy proxy,
                                   ICredentials creds)
        {
            ...
            //if (req.RequestUri.AbsoluteUri.Contains("GravityACS/payauth") || req.RequestUri.AbsoluteUri.Contains("GravityACS/continueWithoutPoints"))
            if (req.RequestUri.AbsoluteUri.Contains("GravityACS/payauth"))
            {
                req.Accept = "*/*";
            }
            else if (req.RequestUri.AbsoluteUri.Contains("json"))
            ...
            //    if (!PreRequest(req))
            //    {
            //        return HttpStatusCode.ResetContent.ToString();
            //    }
            //}
        ...
        private byte[] GetBinary(Uri uri, string method, string path, HtmlDocument doc, IWebProxy proxy,
                                   ICredentials creds)
        {
            string cachePath = null;
            //HttpWebRequest req;
            ...
            if (PreRequest != null)
            {
                ...
                if (!PreRequest(req))
                {
                    return requestState.binaryData;
                }
            }
            ...
            try
            {
                ...
                IAsyncResult result =
                  (IAsyncResult)req.BeginGetResponse(new AsyncCallback(RespCallbackForBinaryOperation), requestState);

                // this line implements the timeout, if there is a timeout, the callback fires and the request becomes aborted
                ThreadPool.RegisterWaitForSingleObject(result.AsyncWaitHandle, new WaitOrTimerCallback(TimeoutCallback), req, _timeout, true);
        ...
        private bool IsCacheHtmlContent(string path)
        {
            ...
            return IfHtmlContent(ct);
        }

        private bool IfHtmlContent(string contentType)
        {
        ...
        private byte[] LoadBinaryUrl(Uri uri, string method, WebProxy proxy, NetworkCredential creds)
        {
            HtmlDocument doc = new HtmlDocument();
            doc.OptionAutoCloseOnEnd = false;
            doc.OptionFixNestedTags = true;
            byte[] data = null;
            data = GetBinary(uri, method, null, doc, proxy, creds);
            if (_statusCode == HttpStatusCode.NotModified)
            {
                // read cached encoding
                doc.DetectEncodingAndLoad(GetCachePath(uri));
    ...
    public class RequestState
    {
        ...
        public RequestState(HttpWebRequest webRequest, HtmlDocument htmlDocument, int tc)
        {
            request = webRequest;
            response = null;
            streamResponse = null;
