...
namespace Automatick.Core
{
    ...
    {
        public static void checkForDistilIdentificationBlock(BrowserSession session, AutoCaptchaServices autoCaptcha, ITicket ticket, AXSSearch search = null)
        {
            try
            {
                HtmlNode captcha = session.HtmlDocument.DocumentNode.SelectSingleNode("//form[@id='distilCaptchaForm']");

                if (captcha != null)
                {
                    if (!processCaptcha(session, autoCaptcha, ticket, search))
                    {
                        return;
                    }
                }
                ...
                {
                    int rfreshCounter = 0;
                    HtmlNode distil = session.HtmlDocument.DocumentNode.SelectSingleNode("//div[contains(@id,'distil_ident_block')]");
                    if (distil == null)
                    {
                        distil = session.HtmlDocument.DocumentNode.SelectSingleNode("//div[contains(@id,'distilIdentificationBlock')]");
                    }
                    if (distil != null)
                    {
                        HtmlNodeCollection scripts = session.HtmlDocument.DocumentNode.SelectNodes("//script");
                        ...
                        if (scripts != null)
                        {
                            foreach (var item in scripts)
                            {
                                if (item.Attributes["src"] != null)
                                {
                                    ...
                                    Uri u = new Uri(session.HTMLWeb.ResponseUri.AbsoluteUri);

                                    Match mRefresh = Regex.Match(session.HtmlDocument.DocumentNode.InnerHtml, "http-equiv=\"refresh\"", RegexOptions.IgnoreCase);
                                    if (!mRefresh.Success)
                                    {
                                        mRefresh = Regex.Match(session.HtmlDocument.DocumentNode.InnerHtml, "http-equiv=refresh", RegexOptions.IgnoreCase);
                                    }

                                    while (mRefresh.Success)
                                    {
                                        BrowserSession _sess = (BrowserSession)session.Clone();
                                        HtmlNode nod = session.HtmlDocument.DocumentNode.SelectSingleNode("//meta[@http-equiv='refresh']");
                                        String content = String.Empty;

                                        if (search != null)
                                        {
                                            if (!search.IfWorking || !search.Ticket.isRunning)
                                            {
                                                break;
                                            }
                                        }
                                        ...
                                        if (rfreshCounter > 0)
                                        {
                                            //_sess._cookies.Clear();
                                            //_sess.TotalCookies.Clear();
                                            //_sess._DISTILL.Clear();
                                        }

                                        mRefresh = Regex.Match(session.HtmlDocument.DocumentNode.InnerHtml, "url=(.*?)\">", RegexOptions.IgnoreCase);
                                        if (mRefresh.Success)
                                        {
                                            _referer = mRefresh.Value.Replace("url=", "").Replace("\">", "");
                                        }
                                        String val = String.Empty;

                                        if (nod != null)
                                        {
                                            val = nod.Attributes["content"].Value.Replace(_referer, String.Empty).Replace("url=", "").Replace(";", String.Empty).Trim();
                                        ...
                                        //if (_referer.Contains("/distil"))
                                        {
                                            ...
                                            //if (search == null || String.IsNullOrEmpty(search.PID_URL))
                                            {
                                                html = _sess.Get(url);


                                                String _url = String.Empty;
                                                String __url = String.Empty;

                                                if (!html.Contains("LegacyFingerprintWrapper("))
                                                {
                                                    _url = html.Substring(html.IndexOf("FingerprintWrapper("));
                                                    __url = _url.Substring(0, _url.IndexOf(")"));

                                                    __url = __url.Replace("FingerprintWrapper(", String.Empty);
                                                }
                                                else
                                                {
                                                    _url = html.Substring(html.IndexOf("LegacyFingerprintWrapper("));

                                                    __url = _url.Substring(0, _url.IndexOf(")"));

                                                    __url = __url.Replace("LegacyFingerprintWrapper(", String.Empty);
                                                }

                                                try
                                                {
                                                    oj = JObject.Parse(__url);
                                                }
                                                catch (Exception ex)
                                                {
                                            ...
                                            //_sess.HTMLWeb.X_Distil_Ajax = oj["ajax_header"].ToString();
                                            //_sess.HTMLWeb.PostData = "p=%7b%22proof%22%3a%2255%3a" + this.Search.UnixTimeNow().ToString() + "%3a" + UniqueKey.getUniqueKey(20) + "%22%2c%22fp2%22%3a%7b%22userAgent%22%3a%22Mozilla%2f5.0+(Windows+NT+6.2)+AppleWebKit%2f537.36+(KHTML%2c+like+Gecko)+Chrome%2f33.0.1750.154+Safari%2f537.36%22%2c%22language%22%3a%22en-US%22%2c%22screen%22%3a%7b%22width%22%3a1920%2c%22height%22%3a1080%2c%22availHeight%22%3a1050%2c%22availWidth%22%3a1920%7d%2c%22timezone%22%3a5%2c%22indexedDb%22%3atrue%2c%22addBehavior%22%3afalse%2c%22openDatabase%22%3afalse%2c%22cpuClass%22%3a%22unknown%22%2c%22platform%22%3a%22Win32%22%2c%22doNotTrack%22%3a%22unspecified%22%2c%22plugins%22%3a%22ShockwaveFlash%3a%3aShockwaveFlash10.3r181%3a%3aapplication%2fx-shockwave-flash%7eswf%2capplication%2ffuturesplash%7espl%22%2c%22canvas%22%3a%7b%22winding%22%3a%22yes%22%2c%22towebp%22%3afalse%2c%22blending%22%3atrue%2c%22img%22%3a%2252bfca0a0b42eac8a74d72cda84e4158b6909ee5%22%7d%2c%22webGL%22%3a%7b%22img%22%3a%228d37e15cc9363584537e76e4d202a7e8e811da59%22%2c%22extensions%22%3a%22ANGLE_instanced_arrays%3bEXT_blend_minmax%3bEXT_color_buffer_half_float%3bEXT_frag_depth%3bEXT_shader_texture_lod%3bEXT_texture_filter_anisotropic%3bEXT_disjoint_timer_query%3bOES_element_index_uint%3bOES_standard_derivatives%3bOES_texture_float%3bOES_texture_float_linear%3bOES_texture_half_float%3bOES_texture_half_float_linear%3bOES_vertex_array_object%3bWEBGL_color_buffer_float%3bWEBGL_compressed_texture_s3tc%3bWEBGL_debug_renderer_info%3bWEBGL_depth_texture%3bWEBGL_draw_buffers%3bWEBGL_lose_context%3bMOZ_WEBGL_lose_context%3bMOZ_WEBGL_compressed_texture_s3tc%3bMOZ_WEBGL_depth_texture%22%2c%22aliasedlinewidthrange%22%3a%22%5b1%2c1%5d%22%2c%22aliasedpointsizerange%22%3a%22%5b1%2c1024%5d%22%2c%22alphabits%22%3a8%2c%22antialiasing%22%3a%22yes%22%2c%22bluebits%22%3a8%2c%22depthbits%22%3a16%2c%22greenbits%22%3a8%2c%22maxanisotropy%22%3a16%2c%22maxcombinedtextureimageunits%22%3a32%2c%22maxcubemaptexturesize%22%3a16384%2c%22maxfragmentuniformvectors%22%3a1024%2c%22maxrenderbuffersize%22%3a16384%2c%22maxtextureimageunits%22%3a16%2c%22maxtexturesize%22%3a16384%2c%22maxvaryingvectors%22%3a30%2c%22maxvertexattribs%22%3a16%2c%22maxvertextextureimageunits%22%3a16%2c%22maxvertexuniformvectors%22%3a4096%2c%22maxviewportdims%22%3a%22%5b32767%2c32767%5d%22%2c%22redbits%22%3a8%2c%22renderer%22%3a%22Mozilla%22%2c%22shadinglanguageversion%22%3a%22WebGLGLSLES1.0%22%2c%22stencilbits%22%3a0%2c%22vendor%22%3a%22Mozilla%22%2c%22version%22%3a%22WebGL1.0%22%2c%22vertexshaderhighfloatprecision%22%3a23%2c%22vertexshaderhighfloatprecisionrangeMin%22%3a127%2c%22vertexshaderhighfloatprecisionrangeMax%22%3a127%2c%22vertexshadermediumfloatprecision%22%3a23%2c%22vertexshadermediumfloatprecisionrangeMin%22%3a127%2c%22vertexshadermediumfloatprecisionrangeMax%22%3a127%2c%22vertexshaderlowfloatprecision%22%3a23%2c%22vertexshaderlowfloatprecisionrangeMin%22%3a127%2c%22vertexshaderlowfloatprecisionrangeMax%22%3a127%2c%22fragmentshaderhighfloatprecision%22%3a23%2c%22fragmentshaderhighfloatprecisionrangeMin%22%3a127%2c%22fragmentshaderhighfloatprecisionrangeMax%22%3a127%2c%22fragmentshadermediumfloatprecision%22%3a23%2c%22fragmentshadermediumfloatprecisionrangeMin%22%3a127%2c%22fragmentshadermediumfloatprecisionrangeMax%22%3a127%2c%22fragmentshaderlowfloatprecision%22%3a23%2c%22fragmentshaderlowfloatprecisionrangeMin%22%3a127%2c%22fragmentshaderlowfloatprecisionrangeMax%22%3a127%2c%22vertexshaderhighintprecision%22%3a0%2c%22vertexshaderhighintprecisionrangeMin%22%3a31%2c%22vertexshaderhighintprecisionrangeMax%22%3a30%2c%22vertexshadermediumintprecision%22%3a0%2c%22vertexshadermediumintprecisionrangeMin%22%3a31%2c%22vertexshadermediumintprecisionrangeMax%22%3a30%2c%22vertexshaderlowintprecision%22%3a0%2c%22vertexshaderlowintprecisionrangeMin%22%3a31%2c%22vertexshaderlowintprecisionrangeMax%22%3a30%2c%22fragmentshaderhighintprecision%22%3a0%2c%22fragmentshaderhighintprecisionrangeMin%22%3a31%2c%22fragmentshaderhighintprecisionrangeMax%22%3a30%2c%22fragmentshadermediumintprecision%22%3a0%2c%22fragmentshadermediumintprecisionrangeMin%22%3a31%2c%22fragmentshadermediumintprecisionrangeMax%22%3a30%2c%22fragmentshaderlowintprecision%22%3a0%2c%22fragmentshaderlowintprecisionrangeMin%22%3a31%2c%22fragmentshaderlowintprecisionrangeMax%22%3a30%7d%2c%22touch%22%3a%7b%22maxTouchPoints%22%3a0%2c%22touchEvent%22%3afalse%2c%22touchStart%22%3afalse%7d%2c%22video%22%3a%7b%22ogg%22%3a%22probably%22%2c%22h264%22%3a%22probably%22%2c%22webm%22%3a%22probably%22%7d%2c%22audio%22%3a%7b%22ogg%22%3a%22probably%22%2c%22mp3%22%3a%22maybe%22%2c%22wav%22%3a%22probably%22%2c%22m4a%22%3a%22maybe%22%7d%2c%22fonts%22%3a%22Calibri%3bCentury%3bHaettenschweiler%3bMarlett%3bPristina%22%7d%2c%22cookies%22%3a1%2c%22setTimeout%22%3a0%2c%22setInterval%22%3a0%2c%22appName%22%3a%22Netscape%22%2c%22platform%22%3a%22Win32%22%2c%22syslang%22%3a%22en-US%22%2c%22userlang%22%3a%22en-US%22%2c%22cpu%22%3a%22WindowsNT10.0%3bWOW64%22%2c%22productSub%22%3a%2220100101%22%2c%22plugins%22%3a%7b%220%22%3a%22ShockwaveFlash10.3.181.26%22%7d%2c%22mimeTypes%22%3a%7b%220%22%3a%22FutureSplashmovieapplication%2ffuturesplash%22%2c%221%22%3a%22AdobeFlashmovieapplication%2fx-shockwave-flash%22%7d%2c%22screen%22%3a%7b%22width%22%3a1920%2c%22height%22%3a1080%2c%22colorDepth%22%3a24%7d%2c%22fonts%22%3a%7b%220%22%3a%22Calibri%22%2c%221%22%3a%22Cambria%22%2c%222%22%3a%22Constantia%22%2c%223%22%3a%22LucidaBright%22%2c%224%22%3a%22Georgia%22%2c%225%22%3a%22SegoeUI%22%2c%226%22%3a%22Candara%22%2c%227%22%3a%22TrebuchetMS%22%2c%228%22%3a%22Verdana%22%2c%229%22%3a%22Consolas%22%2c%2210%22%3a%22LucidaConsole%22%2c%2211%22%3a%22LucidaSansTypewriter%22%2c%2212%22%3a%22CourierNew%22%2c%2213%22%3a%22Courier%22%7d%7d";//"p=" + HttpUtility.UrlEncode("{\"proof\":\"" + UniqueKey.getKey(2) + ":" + this.Search.UnixTimeNow().ToString() + ":" + UniqueKey.getUniqueKey(20) + "\",\"fp2\":{\"userAgent\":\"" + this.session.HTMLWeb.UserAgent.Replace(" ", String.Empty) + "\",\"language\":\"en-US\",\"screen\":{\"width\":1920,\"height\":1080,\"availHeight\":1050,\"availWidth\":1920},\"timezone\":" + TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now) + ",\"indexedDb\":true,\"addBehavior\":false,\"openDatabase\":false,\"cpuClass\":\"unknown\",\"platform\":\"Win32\",\"doNotTrack\":\"unspecified\",\"plugins\":\"ShockwaveFlash::ShockwaveFlash10.3r181::application/x-shockwave-flash~swf,application/futuresplash~spl\",\"canvas\":{\"winding\":\"yes\",\"towebp\":false,\"blending\":true,\"img\":\"52bfca0a0b42eac8a74d72cda84e4158b6909ee5\"},\"webGL\":{\"img\":\"8d37e15cc9363584537e76e4d202a7e8e811da59\",\"extensions\":\"ANGLE_instanced_arrays;EXT_blend_minmax;EXT_color_buffer_half_float;EXT_frag_depth;EXT_shader_texture_lod;EXT_texture_filter_anisotropic;EXT_disjoint_timer_query;OES_element_index_uint;OES_standard_derivatives;OES_texture_float;OES_texture_float_linear;OES_texture_half_float;OES_texture_half_float_linear;OES_vertex_array_object;WEBGL_color_buffer_float;WEBGL_compressed_texture_s3tc;WEBGL_debug_renderer_info;WEBGL_depth_texture;WEBGL_draw_buffers;WEBGL_lose_context;MOZ_WEBGL_lose_context;MOZ_WEBGL_compressed_texture_s3tc;MOZ_WEBGL_depth_texture\",\"aliasedlinewidthrange\":\"[1,1]\",\"aliasedpointsizerange\":\"[1,1024]\",\"alphabits\":8,\"antialiasing\":\"yes\",\"bluebits\":8,\"depthbits\":16,\"greenbits\":8,\"maxanisotropy\":16,\"maxcombinedtextureimageunits\":32,\"maxcubemaptexturesize\":16384,\"maxfragmentuniformvectors\":1024,\"maxrenderbuffersize\":16384,\"maxtextureimageunits\":16,\"maxtexturesize\":16384,\"maxvaryingvectors\":30,\"maxvertexattribs\":16,\"maxvertextextureimageunits\":16,\"maxvertexuniformvectors\":4096,\"maxviewportdims\":\"[32767,32767]\",\"redbits\":8,\"renderer\":\"Mozilla\",\"shadinglanguageversion\":\"WebGLGLSLES1.0\",\"stencilbits\":0,\"vendor\":\"Mozilla\",\"version\":\"WebGL1.0\",\"vertexshaderhighfloatprecision\":23,\"vertexshaderhighfloatprecisionrangeMin\":127,\"vertexshaderhighfloatprecisionrangeMax\":127,\"vertexshadermediumfloatprecision\":23,\"vertexshadermediumfloatprecisionrangeMin\":127,\"vertexshadermediumfloatprecisionrangeMax\":127,\"vertexshaderlowfloatprecision\":23,\"vertexshaderlowfloatprecisionrangeMin\":127,\"vertexshaderlowfloatprecisionrangeMax\":127,\"fragmentshaderhighfloatprecision\":23,\"fragmentshaderhighfloatprecisionrangeMin\":127,\"fragmentshaderhighfloatprecisionrangeMax\":127,\"fragmentshadermediumfloatprecision\":23,\"fragmentshadermediumfloatprecisionrangeMin\":127,\"fragmentshadermediumfloatprecisionrangeMax\":127,\"fragmentshaderlowfloatprecision\":23,\"fragmentshaderlowfloatprecisionrangeMin\":127,\"fragmentshaderlowfloatprecisionrangeMax\":127,\"vertexshaderhighintprecision\":0,\"vertexshaderhighintprecisionrangeMin\":31,\"vertexshaderhighintprecisionrangeMax\":30,\"vertexshadermediumintprecision\":0,\"vertexshadermediumintprecisionrangeMin\":31,\"vertexshadermediumintprecisionrangeMax\":30,\"vertexshaderlowintprecision\":0,\"vertexshaderlowintprecisionrangeMin\":31,\"vertexshaderlowintprecisionrangeMax\":30,\"fragmentshaderhighintprecision\":0,\"fragmentshaderhighintprecisionrangeMin\":31,\"fragmentshaderhighintprecisionrangeMax\":30,\"fragmentshadermediumintprecision\":0,\"fragmentshadermediumintprecisionrangeMin\":31,\"fragmentshadermediumintprecisionrangeMax\":30,\"fragmentshaderlowintprecision\":0,\"fragmentshaderlowintprecisionrangeMin\":31,\"fragmentshaderlowintprecisionrangeMax\":30},\"touch\":{\"maxTouchPoints\":0,\"touchEvent\":false,\"touchStart\":false},\"video\":{\"ogg\":\"probably\",\"h264\":\"probably\",\"webm\":\"probably\"},\"audio\":{\"ogg\":\"probably\",\"mp3\":\"maybe\",\"wav\":\"probably\",\"m4a\":\"maybe\"},\"fonts\":\"Calibri;Century;Haettenschweiler;Marlett;Pristina\"},\"cookies\":1,\"setTimeout\":0,\"setInterval\":0,\"appName\":\"Netscape\",\"platform\":\"Win32\",\"syslang\":\"en-US\",\"userlang\":\"en-US\",\"cpu\":\"WindowsNT10.0;WOW64\",\"productSub\":\"20100101\",\"plugins\":{\"0\":\"ShockwaveFlash10.3.181.26\"},\"mimeTypes\":{\"0\":\"FutureSplashmovieapplication/futuresplash\",\"1\":\"AdobeFlashmovieapplication/x-shockwave-flash\"},\"screen\":{\"width\":1920,\"height\":1080,\"colorDepth\":24},\"fonts\":{\"0\":\"Calibri\",\"1\":\"Cambria\",\"2\":\"Constantia\",\"3\":\"LucidaBright\",\"4\":\"Georgia\",\"5\":\"SegoeUI\",\"6\":\"Candara\",\"7\":\"TrebuchetMS\",\"8\":\"Verdana\",\"9\":\"Consolas\",\"10\":\"LucidaConsole\",\"11\":\"LucidaSansTypewriter\",\"12\":\"CourierNew\",\"13\":\"Courier\"}}");

                                            //html = _sess.Post(oj["path"].ToString());

                                            //_sess.HTMLWeb.PostData = "";
                                            //_sess.HTMLWeb.X_Distil_Ajax = "";

                                            // if (search != null && String.IsNullOrEmpty(search.PID_URL) && String.IsNullOrEmpty(search.X_Distill_Ajax))

                                            if (search != null)
                                            {
                                                search.PID_URL = oj["path"].ToString();
                                                search.X_Distill_Ajax = oj["ajax_header"].ToString();
                                            }

                                            //Debug.WriteLine(oj["path"].ToString());
                                            string cookies = String.Empty;

                                            if (search != null)
                                            {
                                                cookies = getCookiesForFirstPage(session.HTMLWeb.ResponseUri.Scheme + "://" + session.HTMLWeb.ResponseUri.Host + search.PID_URL, search.X_Distill_Ajax, session);
                                            }
                                            else
                                            {
                                                cookies = getCookiesForFirstPage(session.HTMLWeb.ResponseUri.Scheme + "://" + session.HTMLWeb.ResponseUri.Host + oj["path"].ToString(), oj["ajax_header"].ToString(), session);
                                            }

                                            //session.StrCookies = _sess.StrCookies = cookies;//.Replace("Path=/", "").Replace(",", "");
                                            if (!String.IsNullOrEmpty(cookies))
                                            {
                                                cookies = Regex.Replace(cookies, "path=/;,", "", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
                                                cookies = Regex.Replace(cookies, "path=/,", "", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
                                                cookies = Regex.Replace(cookies, "path=/;", "", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
                                                cookies = Regex.Replace(cookies, "Max-Age(.*?);", "", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
                                                cookies = Regex.Replace(cookies, "HttpOnly;", "", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
                                                cookies = Regex.Replace(cookies, "Domain=.axs.com,", "", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);

                                                MatchCollection matchCookies = Regex.Matches(cookies, "(.*?)=(.*?);");
                                                ...
                                                foreach (Match mactchItem in matchCookies)
                                                {
                                                    if (mactchItem.Value.EndsWith(";"))
                                                    {
                                                        ...
                                                        {
                                                            if (!session._DISTILL.ContainsKey(cookieName.Trim()))
                                                            {
                                                                session._DISTILL.Add(cookieName.Trim(), cookieValue.Trim());
                                                            }
                                                            else
                                                            {
                                                                session._DISTILL[cookieName.Trim()] = cookieValue.Trim();
                                                            }
                                                        }
                                                    }
                                                }
                                            ...
                                            //session.HTMLWeb.Referer = session.HTMLWeb.ResponseUri.AbsoluteUri;
                                            //html = session.Get("/distil_identify_cookie.html?uid=" + session._DISTILL["D_UID"] + "&d_ref=" + u.AbsoluteUri);
                                            #endregion
                                        }

                                        session.HTMLWeb.IfAllowAutoRedirect = false;
                                        ////session.HTMLWeb.Referer = session.HTMLWeb.ResponseUri.AbsoluteUri;
                                        html = session.Get(u.AbsoluteUri);

                                        session.HTMLWeb.IfAllowAutoRedirect = true;
                                        if (!string.IsNullOrEmpty(session.RedirectLocation))
                                        {
                                            html = session.Get(session.RedirectLocation);
                                        }

                                        if (search != null)
                                        {
                                            search.changeStatus(TicketSearchStatus.RefreshPageStatus);
                                        ...
                                        //session.Get(_referer);

                                        ////if (_referer.Contains("captcha"))
                                        {
                                            //if (_referer.Contains("captcha") & rfreshCounter == 5)
                                            //{
                                            //    html = _sess.Get(_referer);
                                            //    res = processCaptcha(_sess, autoCaptcha, ticket, search);
                                            //}
                                            //if (_referer.Contains("captcha") & rfreshCounter == 5)
                                            {
                                                captcha = session.HtmlDocument.DocumentNode.SelectSingleNode("//form[@id='distilCaptchaForm']");

                                                if (captcha != null && rfreshCounter < 5)
                                                {
                                                    if (!processCaptcha(session, autoCaptcha, ticket, search))
                                                    {
                                                        return;
                                            ...
                                            {
                                                rfreshCounter++;
                                                scripts = session.HtmlDocument.DocumentNode.SelectNodes("//script");

                                                foreach (var scr in scripts)
                                                {
                                                    if (scr.Attributes["src"] != null)
                                                    {
                                                        url = scr.Attributes["src"].Value;
                                                    }
                                                }
                                            }
                                        }
        ...
        public static void checkForDistillIdentificationBlock(BrowserSession session, AutoCaptchaServices autoCaptcha, ITicket ticket, String scriptValue, AXSSearch search = null)
        {
            try
            {
                HtmlNode captcha = null;
                int rfreshCounter = 0;

                String url = scriptValue;
                String _referer = String.Empty;
                ...
                //while (mRefresh.Success)
                {
                    ...
                    //session.HTMLWeb.IfAllowAutoRedirect = true;
                    //if (!string.IsNullOrEmpty(session.locationHeader))
                    //{
                    //    html = session.Get(session.locationHeader);
                    //}
        ...
        public static void distill(BrowserSession session, AutoCaptchaServices autoCaptcha, ITicket ticket, AXSSearch search = null)
        {
            try
            {
                HtmlNode captcha = session.HtmlDocument.DocumentNode.SelectSingleNode("//form[@id='distilCaptchaForm']");
                            ...
                            foreach (var item in scripts)
                            {
                                if (item.Attributes["src"] != null)
                                {
                                    ...
                                    //   while (mRefresh.Success)
                                    {
                                        ...
                                        ////session.HTMLWeb.Referer = session.HTMLWeb.ResponseUri.AbsoluteUri;
                                        html = session.Get(u.AbsoluteUri);
                                        if (html.Contains("Unable to load user"))
                                        {
                                            Thread.Sleep(3000);
                                            rfreshCounter++;
                                            goto retry;
        ...
        private string getCodefromJS(string url, string jsUrl)
        {
            ...
            try
            {
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url + jsUrl);
                request.Method = "GET";
                ...
                request.UserAgent = "Mozilla/5.0 (iPhone; CPU iPhone OS 7_1_1 like Mac OS X) AppleWebKit/537.51.2 (KHTML, like Gecko) Version/7.0 Mobile/11D201 Safari/9537.53";
                request.AllowAutoRedirect = false;
                response = (HttpWebResponse)request.GetResponse();


                Stream s;
                if (response.ContentEncoding.ToLower().StartsWith("gzip"))
                {
                ...
                string replace = html.Substring(0, html.IndexOf("=") + 2);
                html = html.Replace(replace, "");

                string[] getSecondUrl = html.Split(',');
                html = getSecondUrl[0].Replace("\"", "");
                string distil = getSecondUrl[1].Replace("\"", "");
                //cookies = getCookiesForFirstPage(url + html, distil, session);
        ...
        private static string getCookiesForFirstPage(string url, string XDistilAjax, BrowserSession session)
        {
            HttpWebResponse response = null;
        ...
        static Boolean processCaptcha(BrowserSession session, AutoCaptchaServices autoCaptcha, ITicket ticket, AXSSearch search = null)
        {
            ...
            try
            {
                ...
                HtmlNode recaptchaNode = session.HtmlDocument.DocumentNode.SelectSingleNode("//div[contains(@class,'recaptcha')]");
                if (recaptchaNode == null)
                {
                    do
                    {
                        recaptchaNode = session.HtmlDocument.DocumentNode.SelectSingleNode("//form[contains(@id,'distilCaptchaForm')]");

                        if (recaptchaNode != null)
                        {
                            HtmlNode scriptTag = session.HtmlDocument.DocumentNode.SelectSingleNode("//script[contains(@src,'recaptcha/api/challenge')]");
                            if (scriptTag != null)
                            {
                                String RecaptchaKey = scriptTag.Attributes["src"].Value.Split('=')[1];

                                BrowserSession cloneSession = new BrowserSession();

                                if (session.Proxy != null)
                                {
                                    cloneSession.Proxy = session.Proxy;
                                }

                                String resp = cloneSession.Get(scriptTag.Attributes["src"].Value);

                                if (!String.IsNullOrEmpty(cloneSession.RedirectLocation))
                                {
                                    resp = cloneSession.Get(cloneSession.RedirectLocation);
                                }

                                String challenge = extractParameter(resp, "challenge");
                                String server = extractParameter(resp, "server");
                                String imgURL = server + "image?c=" + challenge;
                                imgBytes = null;
                                ...
                                {
                                    imgBytes = null;//cloneSession.GetBinaryData(imgURL);
                                }

                                Captcha Captcha = new Captcha(imgBytes);

                                if (ticket.ifAutoCaptcha)
                                {
                                    if (search != null)
                                    {
                                        search.changeStatus(TicketSearchStatus.ResolvingCaptchaStatus);
                                    }

                                    if (ticket.ifDBCAutoCaptcha)
                                    {
                                        solveAutoCaptcha = new DeathByCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    }
                                    else if (ticket.ifRDAutoCaptcha)// || ticket.ifRDCAutoCaptcha)
                                    {
                                        //solveAutoCaptcha = new RDCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    }
                                    else if (ticket.ifCPTAutoCaptcha)
                                    {
                                        solveAutoCaptcha = new CPTCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    }
                                    //else if (ticket.ifDCAutoCaptcha)
                                    //{
                                    //    solveAutoCaptcha = new DeCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    //}
                                    //else if (ticket.ifOCR)
                                    //{
                                    //    solveAutoCaptcha = new OCRService(ticket.AutoCaptchaServices, Captcha);
                                    //}
                                    //else if (ticket.ifCAutoCaptcha)
                                    //{
                                    //    solveAutoCaptcha = new CustomCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    //}
                                    //else if (ticket.if2CAutoCaptcha)
                                    //{
                                    //    solveAutoCaptcha = new Captcha2(ticket.AutoCaptchaServices, Captcha);
                                    //}
                                    //else if (ticket.ifAntigateCaptcha)
                                    //{
                                    //    solveAutoCaptcha = new AntigateCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    //}
                                    //else if (ticket.ifAC1AutoCaptcha)
                                    //{
                                    //    solveAutoCaptcha = new CustomCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    //}
                                    else
                                    {
                                        ...
                                        solveAutoCaptcha = new BypassAutoCaptchaService(ticket.AutoCaptchaServices, Captcha);
                                    }
                                ...
                                else
                                {
                                    if (search != null)
                                    {
                                        search.changeStatus(TicketSearchStatus.ManualCaptchaStatus);

                                        AXSTicket tmp = ((AXSTicket)search.Ticket);
                                        lock (tmp.CaptchaQueue)
                                        {
                                            tmp.CaptchaQueue.Add(search);
                                        }

                                        Captcha.captchaentered.WaitOne();
                                    }
                                    else
                                    {
                                ...
                                if (Captcha.CaptchaImage != null)
                                {
                                    Captcha.CaptchaImage.Dispose();
                                    GC.SuppressFinalize(Captcha.CaptchaImage);
                                    Captcha.CaptchaImage = null;
                                }

                                if (!String.IsNullOrEmpty(Captcha.CaptchaWords))
                                {
                                    if (session.FormElements.ContainsKey("recaptcha_response_field"))
                                    {
                                        session.FormElements["recaptcha_response_field"] = Captcha.CaptchaWords;
                                    }
                                    else
                                    {
                                        session.FormElements.Add("recaptcha_response_field", Captcha.CaptchaWords);
                                    }

                                    if (session.FormElements.ContainsKey("recaptcha_challenge_field"))
                                    {
                                        session.FormElements["recaptcha_challenge_field"] = challenge;
                                    }
                                    else
                                    {
                                        session.FormElements.Add("recaptcha_challenge_field", challenge);
                                    }

                                    String postURL = "https://" + session.HTMLWeb.ResponseUri.Host + session.FormElements.Forms[""].Action;

                                    String[] qry = postURL.Split('&');
                                    postURL = "";

                                    foreach (var item in qry)
                                    {
                                        String q = item;
                                        if (item.Equals("httpReferrer="))
                                        {
                                            q = item + session.HTMLWeb.ResponseUri.PathAndQuery;
                                        }
                                        postURL += q + "&";
                                    }

                                    postURL = postURL.TrimEnd('&');

                                    String payload = session.Payload;

                                    session.Payload = "";

                                    //Boolean IFJSON = session._IfJSOn;

                                    //session._IfJSOn = false;

                                    string str = session.Post(postURL);

                                    //session.HTMLWeb.PostData = payload;
                                    //session._IfJSOn = IFJSON;

                                    HtmlNode node = session.HtmlDocument.DocumentNode.SelectSingleNode("//form[@id='distilCaptchaForm']");
                                ...
                                if (search != null)
                                {
                                    search.changeStatus(TicketSearchStatus.CaptchaResolvedStatus);
                                }
                            }
                        }
                        else
        ...
        static string extractParameter(string code, string parameter)
        {
        ...
        public static void RefreshCookies(BrowserSession session, AXSSearch search)
        {
            try
            {
                if (!String.IsNullOrEmpty(search.PID_URL) && !String.IsNullOrEmpty(search.X_Distill_Ajax))
                {
                    String cookies = getCookiesForFirstPage(session.HTMLWeb.ResponseUri.Scheme + "://" + session.HTMLWeb.ResponseUri.Host + search.PID_URL, search.X_Distill_Ajax, session);

                    if (!String.IsNullOrEmpty(cookies))
                    {
                        //if (!String.IsNullOrEmpty(session.StrCookies))
                        //{
                        //    session.StrCookies = session.StrCookies + ", " + cookies;
                        //}
                        //else
                        //{
                        //    session.StrCookies = cookies;
                        //}
